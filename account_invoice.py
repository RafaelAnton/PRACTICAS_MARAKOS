...
MONEDA_SINGULAR = u'DÓLAR'
MONEDA_PLURAL = u'DÓLARES'

CENTIMOS_SINGULAR = u'CÉNTIMO'
CENTIMOS_PLURAL = u'CÉTIMOS'
...
def numero_a_letras(numero):
    ...
    if numero_entero < 0:
        return u'menos %s' % numero_a_letras(abs(numero))
    letras_decimal = u''
    parte_decimal = int(round((abs(numero) - abs(numero_entero)) * 100))
    if parte_decimal > 9:
        letras_decimal = u'punto %s' % numero_a_letras(parte_decimal)
    elif parte_decimal > 0:
        letras_decimal = u'punto cero %s' % numero_a_letras(parte_decimal)
    if (numero_entero <= 99):
    ...
    resultado = resultado.replace(u'uno mil', 'un mil')
    resultado = resultado.strip()
    resultado = resultado.replace(u' _ ', u' ')
    resultado = resultado.replace(u'  ', u' ')
    if parte_decimal > 0:
        resultado = u'%s %s' % (resultado, letras_decimal)
    return resultado


def numero_a_moneda(numero, codigo_moneda):
    if codigo_moneda == u'PEN':
        MONEDA_SINGULAR = u'SOLES'
        MONEDA_PLURAL = u'SOLES'
    else:
        MONEDA_SINGULAR = u'DÓLAR AMERICANOS'
        MONEDA_PLURAL = u'DÓLARES AMERICANOS'
    numero_entero = int(numero)
    parte_decimal = int(round((abs(numero) - abs(numero_entero)) * 100))
    centimos = ''
    if parte_decimal == 1:
    ...
    letras = letras.replace(u'uno', u'un')
    if parte_decimal == 0:
        decimal_str = u'00'
    else:
        decimal_str = str(parte_decimal)
    letras_decimal = u'Y %s/100' % decimal_str
    letras = u'%s %s %s' % (letras, letras_decimal, moneda)
    return letras.upper()
...
class EInvoice(models.Model):
    ...
    correlativo = fields.Char('Correlativo', copy=False)
    tipo_comprobante_id = fields.Many2one('einvoice.catalog.01', string=u'Tipo Documento',
                                          states={'draft': [('readonly', False)]})
    numero_comprobante = fields.Char(u'Número de comprobante', copy=False)
    ...
    codigo_2d = fields.Char(u'Código bidimensional', copy=False)
    total_cadena = fields.Char(u'Monto en letras', copy=False)
    ...
    digest_value = fields.Char('Digest value', copy=False)
    signature_info = fields.Char('Signature info', copy=False)
    enviado = fields.Boolean('enviado a sunat', default=False, copy=False)

    comprobante_xml = fields.Binary(copy=False)
    comprobante_cdr = fields.Binary(copy=False)
    ...
    certificado = fields.Text(string=u'Certificado',
                              default=u"BANANO ORGANICO CERTIFICADO GLOBAL G.A.P  GGN 4052852491785 "
                                      "\n CU: 812842")
    envio_ids = fields.One2many('einvoice.envio', 'einvoice_id', copy=False)
    ...
    contingencia = fields.Boolean(string='Considerar contingencia', default=False, copy=False)
    serie_contingencia = fields.Char(string='Serie de contingencia', copy=False)
    correlativo_contingencia = fields.Char(string='Correlativo de contingencia', copy=False)
    anulada = fields.Boolean(string='Anulada', copy=False)

    # Datos de guia de remision
    gr_enabled = fields.Boolean(u'Asociar con guía de remisión')
    gr_type_id = fields.Many2one('einvoice.catalog.01', u'Tipo de guía', domain=[('code', 'in', ['09', '31'])],
                                 default=lambda self: self.env.ref('odoope_einvoice_base.e01_09').id)
    gr_number = fields.Char(u'Nro de guía')
    ...
    def generar_xml(self):
        for invoice in self:
            ...
            entorno = self.env['ir.config_parameter'].get_param('sunat.billservice_url')
            invoice_xml = ''
            if invoice.tipo_comprobante_id.code == '01' or invoice.tipo_comprobante_id.code == '03':
                ...
                invoice_xml = env.get_template('invoice_2-1.xml').render({'invoice': invoice, 'firmaId': firmaId})
            elif invoice.tipo_comprobante_id.code == '07':
                invoice_xml = env.get_template('nota_credito.xml').render({'invoice': invoice, 'firmaId': firmaId})
            ...
            if entorno == 'produccion':
                resp = requests.post(url=signer_url, data=json.dumps(data),
                                     headers={'Content-Type': 'application/json'})
                firmado = json.loads(resp.text)
            else:
                firmado = dict(signatureValue='1234567890', digestValue='tryingDigestValue123456', comprobante=False)
            invoice.write({
    ...
    def enviar_sunat(self):
        user = self.env['ir.config_parameter'].sudo().get_param('sunat.user')
        password = self.env['ir.config_parameter'].sudo().get_param('sunat.password')
        server_url = self.env['ir.config_parameter'].sudo().get_param('sunat.billservice_url')
        sender_url = self.env['ir.config_parameter'].sudo().get_param('einvoice.sender_url')
        envio_obj = self.env['einvoice.envio'].sudo()
